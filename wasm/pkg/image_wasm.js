 ...
 * @returns {Uint8Array}
 */
export function convert_image(input, target_format) {
 ...
 * @returns {Uint8Array}
 */
export function convert_image_with_options(input, target_format, quality, max_width, max_height, lossless) {
    const ptr0 = passArray8ToWasm0(input, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(target_format, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.convert_image_with_options(ptr0, len0, ptr1, len1, quality, max_width, max_height, lossless);
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
    var v3 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
